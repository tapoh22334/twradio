...
pub async fn request_user_timeline(token: &Oauth2Token, user_id: &str, start_time: Option<&str>) -> Result<twitter_data::TweetsResponse, RequestError> {
    let client = reqwest::Client::new();
    let auth_val = format!("Bearer {}", token.access_token().secret());
    ...
    let url = base_url().join(format!("users/{user_id}/tweets").as_str()).unwrap();
